...
# 格式J：天弘基金（申购/定投，含净值和确认日期，一条短信可包含多笔）
# "您于N月N日通过天弘基金提交的申购/定投<基金名>的申请已成功，申请金额<X>元，确认份额<X>份，确认净值<X>元，确认日期为N月N日"
_PAT_J_UNIT = re.compile(
    r'您于(\d{1,2})月(\d{1,2})日通过天弘基金提交的(?:申购|定投)'
    r'(.+?)的申请已成功，'                    # 基金名称
    r'申请金额为?([\d,]+\.?\d*)元，'          # 金额（"为"可选）
    r'确认份额为?([\d,]+\.?\d*)份，'          # 份额（"为"可选）
    r'确认净值为?([\d.]+)元，'                # 净值（"为"可选）
