...
适用于博时标普、南方纳指等 QDII 基金和招商银行积存金的定投扣款短信：

1. 打开"⚖️ 调仓辅助器"→「📱 短信解析」
2. 将交易确认短信粘贴到文本框（支持批量，多条短信换行分隔）
3. 系统自动识别基金名称、扣款金额、确认份额、净值
4. 确认解析结果无误后点「✅ 填入调仓辅助器」→ **自动完成三件事**：
   - 持仓表对应行 `Shares` += 本次确认份额
   - 持仓表对应行 `Current_Price` = 确认净值
   - 调仓辅助器写入 NCF（买入金额）和 Cash 扣款

> 支持格式：博时标普确认短信、南方纳指扣款短信、招商银行积存金买入短信。不在支持范围的交易用方式 B 手动录入。

**方式 B：手动录入**

适用于短信解析不支持的交易（卖出、新标的买入、港股等）：
