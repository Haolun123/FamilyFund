...
> **状态**：✅ 已实现（财务独立测算 + 储蓄率追踪;调仓决策质量复盘待 transaction.csv 数据积累）
> **日期**：2026-05-02
...
### 定义
```
储蓄率 = 当月投入基金的净现金流 / 税后月均收入
```

### 月均收入口径（广义，年化平摊）

填入 `fi_config.json` 的 `monthly_income_cny` 应使用**广义总收入年化平摊**，而非固定工资：

```
月均收入 = (固定工资 × 12 + 年终奖 + RSU年度归属市值估算) / 12
```

**为什么用广义口径：**
- 奖金和RSU是真实收入，不纳入会导致发奖金月储蓄率虚高、其他月份偏低，曲线锯齿无意义
- FI测算的本质是"用总收入的多少比例在积累资产"，固定工资口径会系统性低估储蓄率

**RSU估算方式：** 用今年预计归属数量 × 当前股价，不需要精确，年底更新一次即可。

### 数据来源
- **投入金额**：`portfolio.csv` Cash 行的正 `Net_Cash_Flow`（外部入金）
- **税后月均收入**：用户手动配置（存入 `fi_config.json`，与财务独立测算共用）

### 注意
- ESPP/RSU 归属的 `Net_Cash_Flow` 不计入"当月储蓄"——已体现在分母（总收入）中，分子只统计主动存入基金的现金
- 只统计 Cash 行的外部入金
