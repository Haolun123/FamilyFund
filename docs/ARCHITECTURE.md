...
│   ├── IMPROVE_LIST.md                # 功能改进清单与设计决策记录
│   ├── DESIGN_BENCHMARK.md            # 基准对比
│   ├── DESIGN_SAP_STOCK.md            # SAP 股票模块
│   ├── DESIGN_PDF_REPORT.md           # PDF 报告
│   ├── DESIGN_MARKET_MONITOR.md       # 市场温度计
│   ├── DESIGN_DAILY_PUSH.md           # EC2 每日推送 + PE 历史快照
│   ├── DESIGN_BACKTEST.md             # 定投回测（含指标解读指南）
│   ├── DESIGN_QUARTERLY_REPORT.md     # 季度家庭财报
│   ├── DESIGN_AI_WEEKLY.md            # AI 周度评估
│   ├── DESIGN_TENTH_MAN.md            # 第十人系统
│   ├── DESIGN_DCA_MANAGER.md          # 定投管理模块
│   ├── DESIGN_AH_MONITOR.md           # AH 溢价监测
│   ├── DESIGN_ANALYTICS.md            # 财务独立 & 储蓄率
│   ├── DESIGN_LIFE_STAGES.md          # 人生阶段规划
│   ├── DESIGN_ACCOUNTING_PRINCIPLES.md# 会计原则（ESPP/RSU NCF 口径）
│   ├── DESIGN_CASHFLOW.md             # 现金流分析（P2，待实现）
│   └── DESIGN_DIVIDEND.md             # 分红处理方案（P3，待实现）
│
├── data/                              # 数据层（iCloud 同步，不入 Git）
│   ├── portfolio.csv                  # ★ 统一资产账本（唯一核心输入）
│   ├── portfolio_sample.csv           # 示例数据（入 Git）
│   ├── own_sap.csv                    # Own SAP (ESPP) 交易记录
│   ├── move_sap.csv                   # Move SAP (RSU) 交易记录
│   ├── balance_sheet.csv              # 季度资产负债表
│   ├── cashflow_log.csv               # 家庭基金外特殊现金流
│   ├── transaction.csv                # 调仓交易记录（P3 决策质量复盘用）
│   ├── sap_price_cache.json           # [程序生成] SAP 股价/汇率缓存
│   ├── market_cache.json              # [程序生成] 市场数据缓存
│   ├── benchmark_cache.json           # [程序生成] 基准数据缓存
...
│   ├── tenth_man_config_GLM.json      # 第十人 GLM 配置（含 API Key）
...
│
├── dashboard/                         # ★ 可视化仪表板
│   └── app.py                         # Streamlit + Plotly（~4000行，8 tabs）
│                                      # Portfolio | Ledger | SAP | Market
│                                      # Backtest | Quarterly | Planning | 10th Man
│
├── scripts/                           # 运维脚本
...
| 文件 | 维护方 | 读写模式 | 版本控制 |
|:---|:---|:---|:---|
| `portfolio.csv` | CIO 手动/UI | 追加快照行 | **不入 Git**（iCloud 同步） |
| `own_sap.csv` / `move_sap.csv` | CIO/UI | 追加交易行 | **不入 Git** |
| `balance_sheet.csv` / `cashflow_log.csv` | CIO 手动 | 追加季度行 | **不入 Git** |
| `transaction.csv` | UI 自动 | 追加交易行 | **不入 Git** |
| `sap_price_cache.json` | 程序自动 | 覆盖写入 | 不入 Git |
| `market_cache.json` / `benchmark_cache.json` | 程序自动 | 覆盖写入 | 不入 Git |
| `yf_symbols.json` | CIO/UI | 增删改 | 不入 Git |
| `ah_config.json` | CIO/UI | 增删改 | 不入 Git |
| `dca_config.json` | CIO/UI | 增删改 | 不入 Git |
| `fi_config.json` | CIO/UI | 增删改 | 不入 Git |
| `life_stages.json` | CIO 手动 | 增删改 | 不入 Git |
| `tenth_man_config_*.json` | 用户配置 | 一次性 | **不入 Git**（含 API Key） |
| `pe_history_us.json` | EC2 自动 | 每日追加 | 不入 Git（EC2 本地积累） |
| `portfolio_sample.csv` | 示例数据 | 只读参考 | 纳入 Git |
| `src/*.py` | 开发维护 | 按需修改 | 纳入 Git |
| `dashboard/app.py` | 开发维护 | 按需修改 | 纳入 Git |
| `scripts/daily_push.py` | 开发维护 | 按需修改 | 纳入 Git |
| `Dockerfile` / `docker-compose.yml` | 开发维护 | 按需修改 | 纳入 Git |
| `.env` | 用户配置 | 一次性 | **不入 Git** |
...
| 基准 | 代码 | 数据来源 | 说明 |
|:---|:---|:---|:---|
| CPI 累计指数 | `cpi` | akshare | 通胀基准线 |
| M2 货币供应量 | `m2` | akshare | 货币扩张基准线 |
| 沪深300 | `csi300` | akshare | A股蓝筹基准 |
| 中证A500 | `csi_a500` | akshare | A股宽基基准 |
| 标普500 (CNY) | `sp500_cny` | yfinance + FX | 美股基准（CNY换算）|
| 纳指100 (CNY) | `ndx100_cny` | yfinance + FX | 美股成长基准（CNY换算）|
| 黄金 (CNY) | `gold_cny` | yfinance + FX | 避险资产基准（CNY换算）|
...
位于 `dashboard/app.py`（~4000行），8个 Tab：
...
| 8 | **10th Man** | 调仓前三 Agent LLM 强制反对审查，支持 GLM/DeepSeek/MiniMax |
...
timeline
    ...
    section Phase 2+3 — 模块化 + 仪表板（✅ 已完成）
        已完成 : Streamlit Dashboard（8 tabs）
             : Plotly 交互式图表
             : SAP 股票成本核算引擎
             : Docker 容器化 + iCloud 数据同步
             : PDF 报告导出
             : XIRR / Sharpe / Calmar / MDD
             : 调仓辅助器 + 数据自动备份
             : 基准对比（7个基准，含CNY换算）
    section Phase 4 — 市场信号（✅ 已完成）
        已完成 : 市场温度计（Market Tab）
              : PE×VIX/QVIX 定投倍数矩阵
              : EC2 每日推送 + PE历史快照
              : DCA 回测引擎（Backtest Tab）
              : 第十人系统（10th Man Tab）
              : AI 周度评估（GLM/DeepSeek/MiniMax）
    section Phase 5 — 家庭资产负债表（✅ 已完成）
        已完成 : 季度财报（Quarterly Tab）
              : AH 溢价监测
              : DCA 定投计划管理
              : PE 历史分位（A股/港股/美股快照）
    section Phase 6 — 财务规划（✅ 已完成）
        已完成 : 人生阶段支出曲线（Planning Tab）
              : 财务独立测算（FI进度/敏感性分析）
              : 储蓄率追踪
              : Tab 重构（8个语义化Tab）
```
...
| 功能 | 说明 | 前置条件 |
|:---|:---|:---|
| **鲨鱼记账解析 + 现金流分析** | 见 `DESIGN_CASHFLOW.md` | 2026Q2 数据 |
| **调仓决策质量复盘** | T+30/90/180 天绩效评估 | transaction.csv 积累3-6个月 |
| **FIFO 已实现盈亏** | 精确成本基础核算 | transaction.csv 积累3-6个月 |
| **现金分红处理** | 见 `DESIGN_DIVIDEND.md` | 待评估 |
...
| `akshare` | A股及宏观数据（CPI/M2/CSI300/PE/QVIX） | 已使用 |
| `yfinance` | 海外市场数据（S&P/NDX/黄金/个股）| 已使用 |
| `scipy` | XIRR 求解 | 已使用 |
| `openai` | 第十人系统 LLM 调用（GLM/DeepSeek/MiniMax）| 已使用 |
