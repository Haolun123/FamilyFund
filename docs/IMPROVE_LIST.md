...
| # | 功能 | 设计文档 | 前置条件 |
|---|------|---------|---------|
| 1 | ~~**鲨鱼记账解析 + 季度现金流分析**~~ | ✅ 完成，见 `src/cashflow_engine.py` + Quarterly Tab | — |
...
| 3b | ~~**短信解析支持新建仓**（未匹配条目 UI 内完成建仓）~~ | ✅ 2026-08-18 完成 | — |
| 7 | **Weekly Update 自动生成 MD 周报** | 见下方详述 | ✅ 2026-08-17 完成 |
| 8 | **合成标普β抵扣强制绑定短信解析** | 见下方详述 | ✅ 2026-08-17 完成 |
| 4 | ~~**修复 7 个 stale tests**~~ | ✅ 2026-08-18 完成，529 passed 0 failed | — |
...
### ~~净资产核对公式：经营性 vs 资本性区分（对应 P2-#6，2026-06-30 发现）~~ ✅ 已修复

已在 2026-07-01 v3 一并修复：`cashflow_engine.py` 中 `CAPITAL_TYPES` 已区分经营性 vs 资本性，资本性流入/流出不计入净资产预测。残差从 -1.27% 收敛到 +0.04%。
...
### 短信解析自定义模板（对应 P2-#9，2026-08-18 记录）

**背景：**
每次遇到新格式短信（新基金公司/渠道）：
- 当前：用户发给 Claude Code → 写正则 → 重启 → 才能自动解析
- 期望：用户自助配置一次，永久生效
...
| 子问题 | 现状 |
|--------|------|
| 解析成功但名字匹配错 | ✅ sms_code_map 已解决（2026-08-18） |
| 新格式第一次解析失败 | 手动填4个字段兜底（parse_error 时展示表单，待实现）|
| 新格式后续解析仍失败 | ❌ 需要自定义模板 or Claude Code 加正则 |

**自定义模板方案设计（待决策）：**

用户贴一条新格式短信，UI 让用户标注日期/基金名/金额/份额/净值各在哪段，系统自动生成正则存入 `sms_patterns.json`，下次同格式自动解析。

**决策依据（用户需思考）：**
- 一年遇到 1-2 次新格式 → 不做，手动填一次 + Claude Code 加正则成本可接受
- 频繁换平台/基金公司 → 做，自定义模板值得投入

**还未做的配套（parse_error 手动填表单）：**
parse_error 条目目前只显示红色错误，用户无法处理。应展示原始短信 + 让用户填：
日期 / 金额 / 份额 / 净值 / 基金名，填完后走正常流程（含 sms_code_map 写入）。
工程量小（~30 行），可独立实施。

### Weekly Update 自动生成 MD 周报（对应 P2-#7）

**触发时机：** 保存快照成功后自动写入，无需手动触发。

**存储路径：** `$FAMILYFUND_DATA/weekly_reports/YYYY-MM-DD.md`（iCloud 同步，可在手机查看）

**报告结构：**

1. **总览**：总资产、单位净值、本周入金，与上周对比
2. **本周交易**：来自 transaction.csv，逐笔列出（日期/操作/标的/金额/份额/净值）
3. **各资产 delta**：
   - 有交易的标的：单独一行，显示市值变化 + 持仓变化
   - 无交易的标的：按类别聚合（美股成长、美股宽基、A股宽基、黄金），只显示市值 delta
4. **合成标普β状态**：预付池余额、本周抵扣金额（来自 `dca_prepaid.json` 的 consumption_log）
5. **资产配置变化**：各类别占比 vs 上周

**数据来源：**
- 本周 / 上周持仓对比：portfolio.csv 最新两个日期的快照
- 本周交易明细：transaction.csv 本周新增行
- 合成标普β状态：`dca_prepaid.json`

**实现位置：** `src/weekly_report.py`（新模块）+ `dashboard/app.py` 保存快照后调用

**修复优先级：** 中

---

### 合成标普β抵扣强制绑定短信解析（对应 P2-#8）

**问题：** 当前抵扣和短信解析是两个独立操作，用户容易先做短信解析保存快照，事后才想起来没做抵扣（2026-08-16 真实发生）。

**期望流程：**
```
短信解析应用
  → 检测到本周有 513400（道指）或 539001（建信纳指）买入
  → 强制展开合成标普β抵扣界面
  → 用户确认抵扣后
  → 才允许进入"保存快照"
```
...
**检查逻辑：**
- 触发条件：`session_state['update_template']` 中本周有道指或建信纳指的 NCF > 0
- 阻断方式：保存按钮 disabled，显示提示"本周有道指/建信买入，请先完成合成标普β抵扣"
- 解除条件：`dca_prepaid.json` 的 `consumption_log` 中存在本周日期的记录

**实现位置：** `dashboard/app.py` Weekly Update 步骤三保存逻辑

**修复优先级：** 中 — 当前靠人工记忆，容易漏

---

### historical_cost 均价算法：券商均价法（对应 P2-#10，2026-08-24 记录）

**背景：**
`historical_cost.py` 当前对所有标的统一使用"纯均价法"（卖出不改变均价）。但不同资产类型的App/券商显示逻辑不同：

| 资产类型 | 卖出后均价变化 | 当前实现 |
|---------|-------------|---------|
| 场外公募基金（南方纳指、博时标普等）| **不变**（申购加权平均）| ✅ 正确 |
| A股个股/场内ETF（成都银行、腾讯、红利低波等）| **会变**（券商移动加权均价法：盈利卖出均价降，亏损卖出均价升）| ❌ 待修复 |

**券商均价法公式：**
```
新均价 = (原总成本 - 卖出份额 × 实际卖出价) / 剩余份额
```
与纯均价法的区别：分子扣的是"实际卖出收益"而非"均价×份额"，使已实现盈亏冲减到剩余持仓成本里。

**实现条件：**
需要 `transaction.csv` 里有完整的卖出记录（Date/Code/Type/Price）。当前 transaction.csv 从2026-04-10开始，历史卖出记录缺失，算不准。

**前置条件：** transaction.csv 积累足够卖出记录（3-6个月），且用户通过对比App持仓成本验证偏差明显后再做。

**当前 workaround：** 初始均价手动从App抄入，边 validate 边调整。
...
### 子基金：儿子教育/安家基金（对应 P2-#11，2026-08-25 记录）

**背景：**
为儿子建立独立的教育/安家基金，现金流来源：每月3000元 + 生日10000元 + 过年30000元 = 年度76000元。

**投资策略（已确定）：**
- 积累期（现在→13岁）：100%纳指100 QDII定投
- 过渡期（14-17岁）：每年将约20%仓位从纳指迁移至固收（建议距用款3年启动，不是5年）
- 用款节点：18岁高等教育 + 25岁安家落户

**回测结论（2026-08-25）：**
基于QQQ 2000-2026年真实历史月度收益率，25年定投：
- 最差情景（2000年科技泡沫顶买入）：**1,773万**（9.3x投入）
- 悲观P10：1,895万
- 中位数：**2,232万**（11.7x）
- 乐观P90：2,542万
- 汇率风险：CNY升值1%/年 vs 贬值1%/年，中位数相差约1000万

**实现方案：独立CSV（方案A）**
- 独立的 `portfolio_son.csv`（iCloud同步），不入主基金
- 因为两个基金可能投资相同标的（如纳指系列），独立CSV避免混淆
