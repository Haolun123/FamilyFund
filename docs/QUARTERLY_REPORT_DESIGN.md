...
## 八、cashflow_log 数据来源：鲨鱼记账 App

**2026-04-20 分析结论**

用户有精确的日常记账习惯（鲨鱼记账 App），可导出 CSV，支持自动化聚合生成 `cashflow_log.csv`，无需手工估算。
...
| 类别 | 金额 | 说明 |
|------|------|------|
| 工资 | ¥333,067 | #严薪水 + #杨薪水（含奖金） |
| 公积金 | ¥96,358 | 含2月一次性补缴 ¥82,459 |
| 房租 | ¥34,520 | 保利天悦 + 九号公馆出租收入 |
| 孩子 | ¥28,033 | 礼金等 |
| **合计** | **¥491,978** | |
...
| 类别 | 金额 | 性质（待确认） |
|------|------|-------------|
| 公司车 | ¥43,588 | ⚠️ 待确认：车贷还款？公司报销？ |
| 贷款 | ¥36,491 | 每月 ¥12,164，⚠️ 待确认：房贷还是Mega车贷？ |
| 孩子 | ¥20,255 | 日常消费 |
| 购物 | ¥19,772 | 日常消费 |
| 餐饮 | ¥13,651 | 日常消费 |
| 阿姨 | ¥11,033 | 家政固定支出 |
...
### 待确认事项（影响 cashflow_log 分类）

1. **公司车 ¥43,588**：是车贷还款（→ `Outflow_Loan`，负债偿还）还是日常车辆费用（→ `Outflow_Major`）？
2. **贷款 ¥36,491**（每月 ¥12,164）：是哪笔贷款？房贷还是 Mega 车贷？→ 应归类为 `Outflow_Loan`，不算消费支出
3. **房租收入 ¥34,520**：是否进入家庭基金盘子？→ 影响 `Inflow_Other` 是否录入
4. **孩子收入 ¥28,033**：礼金性质，归 `Inflow_Other` 还是单独类别？

### 自动化方案（待实现）

确认分类规则后，编写 `scripts/import_cashflow.py`：
- 读取鲨鱼记账 CSV（UTF-16）
- 按类别映射到 cashflow_log Type（`Inflow_Salary`/`Outflow_Major`/`Outflow_Loan` 等）
- 按季度聚合，输出 `data/cashflow_log.csv`
- 每季度末导出一次鲨鱼记账，跑一次脚本，零手工录入
